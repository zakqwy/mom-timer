...
void loop() {
  ...
  if(MODE == 3) { // run
    ...
    display.println(seconds);
    display.setTextSize(1);
    display.print("green to pause");
    display.display();
    if(currentMillis - previousMillis > 1000) { // "calibrated"... vs phone, 8 seconds slow in 17 minutes
  ...
  if(MODE == 4) { // pause
    ...
    display.println("unplug to reset");
    display.display();
    if((buttonStateGreen == LOW) & (buttonStateGreenPrev == HIGH)) {
      MODE -= 1;
      getInputState();
    ...
    if(currentMillis - previousMillis > 200) {
      ...
      if (LEDflashstate == 0) {
        ...
        digitalWrite(LED_RED, LOW);
        LEDflashstate = 1;
        speakerTone = random(200, 600);
        tone(SPEAKER, speakerTone, 200);
      }
      else {
        digitalWrite(LED_BLUE, LOW);
        digitalWrite(LED_RED, HIGH);
        LEDflashstate = 0;
        speakerTone = random(600, 1800);
        tone(SPEAKER, speakerTone, 200);
      }
    }
    if((buttonStateGreen == HIGH) & (buttonStateGreenPrev == LOW)) {
      display.clearDisplay();
      display.display();
      ...
      display.display();
      speakerTone = random(600, 1800);
      for (speakerTone; speakerTone > 60; speakerTone--) {
        tone(SPEAKER, speakerTone, 1);
        delay(1);
      }
      MODE = 0;
      getInputState();
...
void showIntro() {
  ...
  display.setTextSize(1);
  display.println("firmware v0.2");
  display.println("year six fix!");
  display.display();
  ...
  int x = 0;
  getInputState();
  if (scaleInput(potState) == 30) {
    while(scaleInput(potState) != 15) {
      getInputState();
      display.clearDisplay();
      display.setTextColor(WHITE);
      display.setCursor(0,0);
      display.println("Turn knob");
      display.print("to 15 [");
      display.print(scaleInput(potState));
      display.println("]");
      display.println(scaleInput(potState));
      display.display();
      delay(1);
    }
    display.clearDisplay();
